...
CREATE TABLE `mtgapipeline.mtga_silver.matches` (
  ...
  draw_order STRUCT<turns ARRAY<STRUCT<draws ARRAY<INT64>>>> -- The order of cards drawn during the match after the starting hand.
...
CREATE TABLE `mtgapipeline.mtga_silver.decks` (
  ...
  deck_list STRUCT<cards ARRAY<INT64>>,  -- The oracle_id of each card in the deck, repeated for duplicates
  deck_sideboard STRUCT<deck ARRAY<INT64>>, -- The oracle_id of cards in the sideboard. repeated for duplicates, LIMIT LENGTH=15)
...
CREATE TABLE `mtgapipeline.mtga_silver.turn1_hands` (
  ...
  initial_hand STRUCT<init_hand ARRAY<INT64>> NOT NULL, -- The hand drawn, using oracle_id, prior to selecting cards to keep if mulliganCount > 0
  mulliganCount INT64,              -- The mulligan count for the hand (0-7)
  final_hand STRUCT<fin_hand ARRAY<INT64>>, -- The hand after selecting cards to mulligan, using oracle_id
...
CREATE TABLE `mtgapipeline.mtga_silver.pk_counter` (
  ...
  deck_id INT64 NOT NULL,          
  hand_id INT64 NOT NULL
);
